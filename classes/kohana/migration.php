...
abstract class Kohana_Migration {
	...
	protected function migrate($database)
	{
		try
		{
			...
			foreach (self::$statements as $statement)
			{
				if (!$statement->before($this, $database))
				{
					throw new Exception ("Before Hook Returned False");
				}
				$database->query($database::INSERT, $statement->toSQL());
				if (!$statement->after($this, $database))
				{
					throw new Exception ("After Hook Returned False");
				}
			}
			$database->commit();
		}
